  ...
  let compute_transitives pathcond pc =
      ...
      PCSet.fold
        (fun cond acc ->
          ...
          | PathCond.PEquals (v1', v2') ->
              let not_equal1 = PathCond.replace_value pathcond ~src:v1' ~dst:v2' in
              let not_equal2 = PathCond.replace_value pathcond ~src:v2' ~dst:v1' in
              PCSet.add not_equal1 acc |> PCSet.add not_equal2
          | _ ->
              acc)
        pc_set PCSet.empty
    in
    let replace_pc_set_by_pathcond =
      ...
      | PathCond.PEquals (v1, v2) ->
          ...
          pc_set |> PCSet.union pc_set_to_add1 |> PCSet.union pc_set_to_add2
      | _ ->
          pc_set
    in
